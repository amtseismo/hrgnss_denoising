...
MODEL 2: Train a CNN to denoise 3 component GNSS data by predicting a complex mask
...
if train:
    if not(norm_input):
        model_save_file="quickie_3comp_v2_"+str(fac)+"_"+str(datetime.datetime.today().month)+"-"+str(datetime.datetime.today().day)+".tf"
    else:
        model_save_file="quickie_3comp_norm_input_v2_"+str(fac)+"_"+str(datetime.datetime.today().month)+"-"+str(datetime.datetime.today().day)+".tf"
else:
    if not(norm_input):
        model_save_file="quickie_3comp_v2_"+str(fac)+"_8-18.tf"  
    else:
        model_save_file="quickie_3comp_norm_input_v2_"+str(fac)+"_8-18.tf"     
...
my_data=gnss_tools.stft_3comp_data_generator('v2',32,x_data[x_train_inds,:],n_data[n_train_inds,:],sr,nperseg,noverlap,norm_input)
x,y=next(my_data)
my_test_data=gnss_tools.stft_3comp_data_generator('v2',50,x_data[x_train_inds,:],n_data[n_train_inds,:],sr,nperseg,noverlap,norm_input,valid=True)
x,y,sigs,noise,x1=next(my_test_data)

# # PLOT GENERATOR RESULTS
# print("PLOT GENERATOR RESULTS")
# gnss_tools.plot_generator_results('v2',t,f,x,y,sigs,noise,x1,nlen,sr,nperseg,noverlap,norm_input)
        
# BUILD THE MODEL
print("BUILD THE MODEL")
model=gnss_tools.make_small_unet_v2(drop=drop,ncomp=3,fac=fac)
...
if train:
    ...
    history=model.fit_generator(gnss_tools.stft_3comp_data_generator('v2',batch_size,x_data[x_train_inds,:],n_data[n_train_inds,:],sr,nperseg, noverlap, norm_input),
                        steps_per_epoch=(len(x_train_inds))//batch_size,
                        validation_data=gnss_tools.stft_3comp_data_generator('v2',batch_size,x_data[x_test_inds,:],n_data[n_test_inds,:],sr,nperseg, noverlap, norm_input),
...
my_test_data=gnss_tools.stft_3comp_data_generator('v2',maxrange,x_data[x_test_inds,:],n_data[n_test_inds,:],sr,nperseg,noverlap,norm_input,valid=True)
x,y,sigs,noise,x1=next(my_test_data)
test_predictions=model.predict(x)

# GET DENOISED SIGNALS FROM OUTPUT
print("GETTING DENOISED SIGNALS FROM MODEL OUTPUT")
tru_sig_inv, tru_noise_inv, est_sig_inv, est_noise_inv=gnss_tools.output_2_data('v2',y,x1,test_predictions,sr,nperseg,noverlap, norm_input)

# SAVE SUBSET FOR PLOTTING PURPOSES
goto=maxrange
with open('model2_v'+str(fac)+'_results.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
